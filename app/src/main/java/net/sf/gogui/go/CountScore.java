...
public class CountScore
{
    ...
    public PointList changeStatus(GoPoint p)
    {
        ...
        if (c == EMPTY) {
        	 ...
        	 if (emptyRegionSize(p, stones) <= 2) {
        		 for (GoPoint adj : stones) 
        			 m_dame.set(adj, !m_dame.get(adj));  // TODO test this
        	 } else {
        		 stones = new PointList();
        	 }
        	 return stones;
        }
    ...
    */
    ...
    public GoColor getColor(GoPoint p, ScoringMethod rules) 
    {
    ...
    */
    ...
    private int emptyRegionSize(GoPoint p, PointList stones) {
        Marker marker = new Marker(m_board.getSize());
        findEmptyRegion(p, marker, stones);
        return stones.size();
    }
    
    private void findEmptyRegion(GoPoint p, Marker marker,
            PointList stones) {
		if (marker.get(p))
			return;
		GoColor c = m_board.getColor(p);
		...
		for (GoPoint adj : m_board.getAdjacent(p))
			findEmptyRegion(adj, marker, stones);
    }
