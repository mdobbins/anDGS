...
public class SgfWriter
{
    ...
    */
    ...
    private static final int STRINGBUF_CAPACITY = 256; // 128;
    ...
    private void printLabels(ConstNode node)
    {
        ...
        if (labels == null || labels.size() == 0)   // anDGS to avoid 0 length list
            return;
        StringBuilder buffer = new StringBuilder(STRINGBUF_CAPACITY);
        buffer.append("LB");
    ...
    private void printMarked(ConstNode node, String property, MarkType type)
    {
        ...
        if (marked != null) {
        	if (marked.size() > 0) {	// added anDGS can create null lists
    ...
    {
    	/* modification to remove recursive decent on a single child as this tends to blow the stack */
       while (true) {  /* added */
        ...
        if (sgfProps != null) {
            ...
            {  // for anDGS build entire property to put on one line
            	StringBuilder buffer = new StringBuilder(STRINGBUF_CAPACITY);
                ...
                for (int i = 0; i < sgfProps.getNumberValues(key); ++i)
                	buffer.append("[").append(sgfProps.getValue(key, i)).append("]");
